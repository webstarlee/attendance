...
class AttendanceController extends Controller
{
    ...
    {
        $request_attend = AttedanceRequest::find($request->attendance_request_id);
        if ($request_attend) {
            $employee = User::find($request_attend->employee_id);
            $contract_type = ContractType::find($employee->contract_type);

            $time1 = $this->decode_time_format($request->_request_attend_start_time);
            $time2 = $this->decode_time_format($request->_request_attend_end_time);

            $cal_min = $this->calculate_time_minute($time1) - $this->calculate_time_minute($time2);

            $selected_min = abs($cal_min);

            if ($request->_request_attend_fix_time && $selected_min > $contract_type->working_time) {
                return "fail_3";
            }

            $daterange = $this->createDateRange($request->_request_attend_date_from, $request->_request_attend_date_to);

            foreach ($daterange as $date) {

                $current_attend_type = $request->_request_attendance_type;

                $check_attendances = Attendance::where('attend_date', $date)->where('employee_id', $employee->id)->get();
                ...
                $is_weekend = $this->isWeekend($date);
                if ($request->_request_attend_weekend && $is_weekend) {
                    $boolean_check = false;
                }

                $is_holiday = $this->isHoliday($date);
                if ($request->_request_attend_holiday && $is_holiday) {
                    $boolean_check = false;
                }
                ...
                if ($boolean_check) {
                    $attendance = new Attendance;
                    $attendance->employee_id = $request_attend->employee_id;
                    $attendance->attend_date = $date;
                    $attendance->attend_type = $request->_request_attendance_type;
                    $attendance->start_time = $this->decode_time_format($request->_request_attend_start_time);
                    $attendance->end_time = $this->decode_time_format($request->_request_attend_end_time);
                    $attendance->save();
